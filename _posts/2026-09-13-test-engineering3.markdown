---
layout: post
title: "业务模型即Spec：AI时代的SDT实践与MBT民主化"
date: 2026-09-13
...
三十年来，MBT始终停留在“实验室技术”和“高合规行业专属工具”的位置。原因并不在技术本身，而在于它的三重门槛：
...
以上痛点这就是MBT最典型的适用场景，在疲于应付复杂的状态机变化导致的需求变化之时，团队对MBT的诉求呼之欲出。当时已经是LLM大规模出现“涌现”现象的阶段。测试开发负责人在评估测试自动化方案的时候开始认真考虑MBT的可行性。

模型擅长快速实现算法开发，可以通过建模，将业务问题转换成算法问题。这中间的桥梁，就是算法和模型都易解析的YAML文件
...
- **业务专家能看懂**：不需要懂UML，看描述就知道状态怎么转
- **可版本控制**：和代码一起进Git，变更可追溯
- **可自动处理**：机器直接读入进行图分析
...
一旦把状态机抽象为图，一大堆成熟的图论概念就能派上用场：

强连通分量（SCC）：表示一组状态之间可以两两互相到达。注意，不要求两两直接相连，只要求存在路径。在状态机中，SCC对应**状态循环**——例如 running ↔ stopped 可以互相转换，构成一个强连通分量。
...
**有了以上知识，我们就可以对照业务设计YAML文件结构，保证文件内定义的字段准确描述了所有的业务关系及变化。**
...
**有了以上知识，我们可以寻找合适的python数学计算工具，无需从头实现算法。**

---

## 四、图论到算法：Vibe Coding全搞定

### 4.1 NetworkX：图算法的标准库
...
### 4.3 Vibe Coding：让AI写算法，业务事实验证算法

到这里，一个完整的分析脚本就呼之欲出了。但真正让MBT民主化的，是**Vibe Coding**——用自然语言描述需求，让LLM生成代码。
...
LLM会在几分钟内生成一个完整可用的框架。

这是MBT民主化的第三个支点：不再需要用一个通用工具解决所有问题，开发成本从“月”降到“小时”，维护成本从团队降到个人。

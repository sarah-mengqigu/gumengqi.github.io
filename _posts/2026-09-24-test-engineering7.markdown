...
如果研发团队都采用AI Native的方式，是否还需要用AI Native方式维护用例集，如果不维护，如何定义确定性的质量基线，如何抑制测试过程中的熵增也需要在实践中摸索。
...
而一些有大量技术沉淀的团队构建维护的应用、一些更复杂的分布式应用都无法一套简单的数据模型解释所有的业务。就需要整个行业在实践中探索出更多元化的方案。


软件工程领域对类似问题的探索有两个大方向可以借鉴：知识溯源，Agent Harness
